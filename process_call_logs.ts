...
async function fetchAgentIds(): Promise<string[]> {
  ...
  return (data || []).map((row: any) => row.agent_id).filter(Boolean);
}

async function fetchCallsFromApi(agentId: string): Promise<any[]> {
...
async function callExists(call_id: string): Promise<boolean> {
  const { data, error } = await supabase
    .from('call_logs')
    .select('call_id')
    .eq('call_id', call_id)
...
async function leadExists(call_id: string): Promise<boolean> {
  const { data, error } = await supabase
    .from('leads')
    .select('call_id')
    .eq('call_id', call_id)
...
function normalizeDateFields(obj: any, dateFields: string[]) {
  const out = { ...obj };
...
async function extractFromTranscript(call: any) {
...
async function extractLeadFromTranscript(transcript: string): Promise<any | null> {
  const prompt = `
...
async function extractContactInfoAndSummaries(transcript: string): Promise<any> {
...
async function upsertLead(call: any, intent: string, agent_id: string, extracted?: any) {
  if (!(intent === 'Service' || intent === 'Emergency' || intent === 'Quotation')) return 'skipped';
  ...
  const lead = filterFields(
    normalizeDateFields(leadRaw, ['appointment_date', 'created_at', 'updated_at']),
    LEADS_FIELDS
  );
...
async function upsertCallLog(call: any, agent_id: string) {
